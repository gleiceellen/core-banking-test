...
> 2. Escolha a branch com a stack de sua preferência — `kotlin` ou `java`. Após criar o repositório, clone-o e rode `git checkout <branch-escolhida>` (ex.: `git checkout java`). Para evitar confusão, considere apagar a outra branch e definir a escolhida como padrão em Settings → Branches.
...
| Testes | JUnit 5, Mockito, ArchUnit (teste de arquitetura), MockMvc |
...
- `domain/model/Greeting.java` — a saudação já renderizada, pronta para resposta.
- `domain/model/GreetingTemplate.java` — um template de saudação (`id` + `template` com placeholder `%s`).
- `domain/exception/BlankRequesterNameException.java` — nome do solicitante em branco.
- `domain/exception/InvalidGreetingTemplateException.java` — template inválido (id ou texto em branco).
