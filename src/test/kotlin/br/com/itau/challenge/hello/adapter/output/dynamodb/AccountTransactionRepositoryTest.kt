...
class AccountTransactionRepositoryTest {
    ...
    fun `should create account if absent`() {
        ...
        `when`(client.putItem(any<software.amazon.awssdk.services.dynamodb.model.PutItemRequest>())).thenReturn(software.amazon.awssdk.services.dynamodb.model.PutItemResponse.builder().build())
    ...
    fun `should ignore create when already exists`() {
        val client = mock(DynamoDbClient::class.java)
        `when`(client.putItem(any<software.amazon.awssdk.services.dynamodb.model.PutItemRequest>()))
    ...
    fun `should throw currency mismatch`() {
        ...
        `when`(client.getItem(any<GetItemRequest>())).thenAnswer { inv ->
    ...
    fun `should detect idempotency conflict`() {
        ...
        val client = mock(DynamoDbClient::class.java)
        `when`(client.getItem(any<GetItemRequest>())).thenAnswer { GetItemResponse.builder().item(existing).build() }
        val repo = AccountTransactionRepository(client, "core_banking")
    ...
    fun `should return idempotent same payload with current balance`() {
        ...
        val client = mock(DynamoDbClient::class.java)
        var call = 0
        `when`(client.getItem(any<GetItemRequest>())).thenAnswer { _ ->
            call++
    ...
    fun `should save credit successfully`() {
        ...
        `when`(client.transactWriteItems(any<software.amazon.awssdk.services.dynamodb.model.TransactWriteItemsRequest>())).thenReturn(software.amazon.awssdk.services.dynamodb.model.TransactWriteItemsResponse.builder().build())
        val repo = AccountTransactionRepository(client, "core_banking")
        val (resp, bal) = repo.save(txId, req)
        assertEquals(TransactionStatus.SUCCEEDED, resp.status)
    ...
    fun `should handle insufficient balance as FAILED`() {
        ...
        `when`(client.transactWriteItems(any<software.amazon.awssdk.services.dynamodb.model.TransactWriteItemsRequest>())).thenThrow(ConditionalCheckFailedException.builder().message("cond").build())
        val repo = AccountTransactionRepository(client, "core_banking")
        val (resp, bal) = repo.save(txId, req)
